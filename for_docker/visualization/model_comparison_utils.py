...
def load_all_histories(base_path="../model_save_preset"):
    ...
    # Преобразуем путь в объект Path для корректной работы с Unicode/кириллицей
    ...
    for group_dir in sorted(all_groups):
        ...
        if os.path.isdir(group_path):
            ...
            try:
                ...
                print(f"  Ошибка при чтении директории {group_path}: {e}")
                continue
                    ...
                    try:
                        ...
                        try:
                            if history_file.endswith('.pkl'):
                                ...
                                # Явно используем UTF-8 для всех JSON-файлов
                                try:
                                    ...
                                    except Exception as json_latin_err:
                                        print(f"  Ошибка загрузки JSON файла с latin-1 кодировкой {history_file}: {json_latin_err}")
                                        # Пробуем еще cp1251 (кириллица Windows)
                                        try:
                                            with open(history_file_path, 'r', encoding='cp1251') as f3:
                                                history_data = json.load(f3)
                                        except Exception as json_cp1251_err:
                                            print(f"  Ошибка загрузки JSON файла с cp1251 кодировкой {history_file}: {json_cp1251_err}")
                                            continue
                                except Exception as json_err:
                                    print(f"  Ошибка загрузки JSON файла {history_file}: {json_err}")
                                    continue
