# 3. alt_models/1/alt1_gru_wrapper.py
from pathlib import Path, PurePath
import numpy as np, tensorflow as tf

_ROOT = Path(__file__).parent
cands = sorted(_ROOT.glob("model_gru*.h5"), key=lambda p: p.stat().st_mtime, reverse=True)
if not cands:
    raise FileNotFoundError("Не найдено ни одного веса GRU в saved_models/")
_WEIGHTS = cands[0]
