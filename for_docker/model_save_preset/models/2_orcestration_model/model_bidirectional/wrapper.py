# 3. alt_models/1/alt1_bidirectional_wrapper.py
from pathlib import Path
import numpy as np
import tensorflow as tf

_ROOT = Path(__file__).parent
cands = sorted(_ROOT.glob("model_bidirectional*.h5"), key=lambda p: p.stat().st_mtime, reverse=True)
if not cands:
    raise FileNotFoundError("Не найдено ни одного веса Bidirectional в saved_models/")
_WEIGHTS = cands[0]
